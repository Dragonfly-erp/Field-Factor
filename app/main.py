...
class Обробник(BaseHTTPRequestHandler):
    ...
    def _задача(self, дані):
        ...
        elif режим == "зони_врожайність":
            if not СЕАНС.підсумок:
                return self._збій("Спершу треба нормалізована карта врожайності")
            ціль = tasks.зонування
            аргументи = (СЕАНС, КАРТОТЕКА, г, п, дані.get("зони") or {})
        else:
            return self._збій("Режим «{}» ще не зроблений".format(режим))

        СЕАНС.потік = threading.Thread(target=ціль, args=аргументи, daemon=True)
        СЕАНС.потік.start()
        return self._віддати({"гаразд": True})
