...
def метрична_crs(gdf):
    ...
    Центр береться з МЕЖ, а не зі злиття геометрій. Злиття тут коштувало
    дорого двічі: на сотні тисяч точок монітора воно рахується довго, а на
    полігоні з самоперетином падає з `TopologyException` — і зона не
    підбирається взагалі, хоч для вибору зони достатньо приблизного центру.
    """
    у_градусах = gdf.to_crs(4326)
    xmin, ymin, xmax, ymax = у_градусах.total_bounds
    центр_x, центр_y = (xmin + xmax) / 2.0, (ymin + ymax) / 2.0
    # Ворота на безглузді координати. Шар, у якого написано «градуси», а
    # лежать метри, дає центр на кшталт 4 400 000 — і зона виходить
    # тризначною. Мовчки повернути таку «CRS» означає порахувати площу
    # на вигаданій проєкції; краще спинитись і сказати, що саме не так.
    if not (-180.0 <= центр_x <= 180.0 and -90.0 <= центр_y <= 90.0):
        raise ValueError(
            "координати не схожі на градуси: центр {:.1f}, {:.1f} — "
            "шар підписаний EPSG:4326, але лежить в іншій системі"
            .format(центр_x, центр_y))
    зона = int((центр_x + 180) // 6) + 1
    зона = min(60, max(1, зона))
    південна = центр_y < 0
